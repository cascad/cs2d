...
/// Сессионная сущность игровой сцены: живёт от входа в игру до выхода в меню.
/// Всем, что спавнят OnEnter(InGame)-сетапы (уровень, HUD, подсветки, маркеры
/// прицела), и мировым объектам с временем жизни «до конца сессии» (трупы,
/// декали) — ставить этот маркер. `despawn_session_scene` (OnExit) чистит всё
/// разом: без этого каждый реконнект (АФК-кик, обрыв) спавнил ВТОРЫЕ копии, и
/// системы с `single()` молча отключались — контур удара замерзал на полу,
/// строки табло очков пропадали, кольцо гранаты отваливалось.
#[derive(Component)]
