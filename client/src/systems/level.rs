...
pub fn spawn_level(mut commands: Commands) {
    ...
    commands.spawn((
        Sprite {
            color: wall_color,
            custom_size: Some(Vec2::new(WALL_THICKNESS, LEVEL_HEIGHT + WALL_THICKNESS * 2.0)),
            ..default()
        },
    ...
    commands.spawn((
        Sprite {
            color: wall_color,
            custom_size: Some(Vec2::new(LEVEL_WIDTH + WALL_THICKNESS * 2.0, WALL_THICKNESS)),
            ..default()
        },
    ...
    // Центральная вертикальная стена
    commands.spawn((
        Sprite {
            color: wall_color,
    ...
    info!(target: "level", "spawn wall at {:?}", Transform::from_translation(Vec3::ZERO).translation);
}
