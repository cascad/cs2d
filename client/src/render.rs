...
pub mod layers {
    ...
    /// Просвечивающие пропы (бочки, сундуки) — НИЖЕ трупов, чтобы тело лежало
    /// поверх них. Они «низкие/прозрачные», поэтому не должны перекрывать лежащих.
    pub const PROP_SEETHROUGH: f32 = 50.0;
    pub const CORPSE: f32 = 150.0;
    /// Стены — ВЫШЕ трупов (тело лежит ЗА стеной), но ниже актёров (живой стоит
    /// перед стеной). Колонны — глухие пропы — рисуются в слое актёров (ACTOR).
    pub const WALL: f32 = 175.0;
