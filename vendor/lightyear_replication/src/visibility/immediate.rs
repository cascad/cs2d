    ...
    fn update_network_visibility(
        ...
        query.iter_mut().for_each(|mut state| {
            state.per_sender_state.retain(|sender_entity, state| {
                // PATCH(cs2d): линк клиента (sender) мог быть уже удалён —
                // netcode деспавнит его при дисконнекте, а per-sender записи
                // доживают до этой системы. Оригинальный `.unwrap()` ронял
                // сервер паникой при выходе клиента. Запись мёртвого
                // отправителя просто выбрасываем.
                let Ok(sender) = manager_query.get(*sender_entity) else {
                    return false;
                };
