        ...
        world.commands().queue(move |world: &mut World| {
            ...
            match &replicate.mode {
                ...
                ReplicationMode::SingleServer(target) => {
                    ...
                    // PATCH(cs2d): в мире может быть НЕСКОЛЬКО Server-сущностей
                    // (UDP- и WebTransport-листенеры). Оригинальный `.single()`
                    // при этом всегда падал и молча выходил — флаги
                    // predicted/interpolated не проставлялись НИ ОДНОМУ клиенту.
                    // Собираем линки со всех серверов.
                    let clients: Vec<Entity> = server_world
                        .query_filtered::<&Server, With<Server>>()
                        .iter(server_world)
                        .flat_map(|server| server.collection().iter().copied())
                        .collect();
                    if clients.is_empty() {
        ...
        world.commands().queue(move |world: &mut World| {
            ...
            match &replicate.mode {
                ...
                ReplicationMode::SingleServer(target) => {
                    ...
                    // SAFETY: we will use this to access the server-entity, which does not alias with the ReplicationSenders
                    let world = unsafe { unsafe_world.world_mut() };
                    // PATCH(cs2d): поддержка нескольких Server-сущностей (UDP +
                    // WebTransport): `.single()` при двух листенерах молча выходил,
                    // и per-sender состояние репликации не создавалось.
