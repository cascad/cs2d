...
use crate::{
    resources::{SolidTiles, SpawnPoints, VisionGridRes, WallAabbs, WallGridRes},
    systems::wall::Wall,
};
...
pub fn create_fixed_level(
    ...
    (SolidTiles(solid), lvl.spawns, lvl.wall_aabbs, lvl.vision_aabbs)
}

/// Системный сетап: один раз строим уровень и кладём ресурсы
pub fn setup_fixed_level(mut commands: Commands) {
    let (solid, spawns, wall_aabbs, vision_aabbs) = create_fixed_level(&mut commands);

    // Сетка ДВИЖЕНИЯ — все стены и пропы; сетка ВЗГЛЯДА — без низких пропов.
    let grid = WallGrid::build(&wall_aabbs, WALL_GRID_CELL);
    let vision_grid = WallGrid::build(&vision_aabbs, WALL_GRID_CELL);
